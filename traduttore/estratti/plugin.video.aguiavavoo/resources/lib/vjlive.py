...
CACHE_TIMEOUT = 10800         
COUNTRY_CACHE_TIMEOUT = 18000 
REQUEST_TIMEOUT = 10         

API_BASE_URL = "https://vavoo.to"
INDEX_URL = "https://vavoo.to/live2/index"
...
def getchannels(selected_groups=None, use_cache=True):
    ...
    if "Portugal" in selected_groups:
        utils.log_debug("Adding Portugal  channels from Balkans group", "getchannels")
...
def livePlay(name, urls=None, group=None):
    ...
    progress.create('[COLOR yellow]Águia Vavoo[/COLOR]', 
                '[COLOR orange]O streaming vai iniciar divertir com o Águia Vavoo[/COLOR]')
    progress.update(30)
    ...
    if auto_play:
        ...
        for idx in range(total):
            ...
            progress.update(pct, '[COLOR yellow]O streaming vai iniciar, diverte-te com o Águia Vavoo[/COLOR]')
        ...
        progress.update(50, '[COLOR yellow]O streaming vai iniciar, diverte-te com o Águia Vavoo[/COLOR]')
    ...
    display_name = f"[COLOR yellow]Águia Vavoo[/COLOR] [COLOR orange]{name}[/COLOR]"
    listitem = xbmcgui.ListItem(display_name)
    addon_icon = utils.addon.getAddonInfo('icon')
    listitem.setArt({
        "icon": addon_icon,
        "thumb": addon_icon,
        "poster": addon_icon,
    ...
    base_name = f"[COLOR yellow]Águia Vavoo[/COLOR] [COLOR orange]{name}[/COLOR]"
    display_title = f"{base_name} ({final_index + 1}/{total})" if total > 1 else base_name
...
def makem3u(group):
    ...
    progress = xbmcgui.DialogProgress()
    progress.create("[COLORorange]Águia [COLORyellow]Vavoo[/COLOR]", f"Fetching channels for {group}...")
    ...
    filename = f"[COLORred]Águia [COLORwhite]Vavoo[/COLOR]-{clean_group}-{date_str}.m3u"
